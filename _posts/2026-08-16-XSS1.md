...
二、逐关解题思路
题目已经给出了提示  过关目标为利用xss漏洞在页面执行alert()函数，后续查看源代码注意这个。
...
回车后通关。
![截图3](屏幕截图 2026-08-17 145103.png)
原理：
...
失败原因分析：
查看网页源码可以发现关键代码：
![截图5](屏幕截图 2026-08-17 145248.png)
escape() 函数把 <、>、" 全部转成了 %xx 编码。浏览器看到 %3Cscript%3E 只会当纯文本显示，HTML 标签注入这条路被堵死了。
但是注意看源代码，我们的输入被放在了一对‘’里面，这是天无绝人之路啊！
中所周不知。escape() 会转义 < > "，但不会转义单引号 '。既然不能跳出 HTML，那就在 JS 内部逃逸。
构造payload：
?username=';alert(1)//
...
回车之后就可以通过这一关了。
![截图6](屏幕截图 2026-08-17 145329.png)

Level 3：DOM XSS —— 去掉 escape 后
场景分析：
查看源码。
![截图7](屏幕截图 2026-08-17 145400.png)
注意这一段：
document.getElementById('ccc').innerHTML = "Welcome " + username;

我们看出escape() 被去掉了！ 输入不再被编码，< > 会原样进入 DOM。
所以直接复现第一关的思路，构造payload：
?username=<script>alert(1)</script>
但是题目过滤了 script 关键字，。。。。牛逼简直。。。
所以我们换一种方式，构造：
?username=<img src=x onerror=alert(1)>
回车后通关。
![截图8](屏幕截图 2026-08-17 145503.png)

Level 4：javascript: 伪协议 —— 定向执行代码
场景分析：
查看源代码，发现页面跳转的控制代码。
![截图9](屏幕截图 2026-08-17 145602.png)
...
等倒计时结束就通关啦啊啦啦啦！！！
![截图10](屏幕截图 2026-08-17 145646.png)

Level 5：表单 action 劫持 —— 伪协议的变体
烦死了还没完。。。。
场景分析：
页面是表单提交页，所以我们查看源码
![截图11](屏幕截图 2026-08-17 145710.png)
![截图12](屏幕截图 2026-08-17 145825.png)
阅读代码后我们发现：
...
回车通关！
![截图13](屏幕截图 2026-08-17 145856.png)
...
通关得到flag！
![截图14](屏幕截图 2026-08-17 145936.png)
